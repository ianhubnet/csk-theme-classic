...
(function () {
	...
	once_action('theme_setup', function ($ci) {
		...
		if (CI_LIVE) {
			$ci->hub->assets->css($ci->url->theme('assets/css/style.min.css'), 'style', null, true);
		} else {
			$ci->hub->assets->css($ci->url->theme('assets/css/style.css'), 'style', null, true);
		}
	});
	 ...
	 */
	once_filter('captcha_img_height', fn($height) => 36);
